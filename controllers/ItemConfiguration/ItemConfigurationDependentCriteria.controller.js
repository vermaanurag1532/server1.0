import ItemConfigurationDependentCriteriaService from '../../Service/ItemConfigration/ItemConfigurationDependentCriteria.service.js'
import { generateAndDownloadExcel } from '../../utils/excelUtils.js';

const ItemConfigurationDependentCriteriaControllers = {
    getItemConfigurationItemType: async (req, res) => {
        try {
            const results = await ItemConfigurationDependentCriteriaService.getAllItemConfigurations();
            res.json(results);
        } catch (error) {
    ...
    postItemConfigurationItemType: async (req, res) => {
        try {
            const result = await ItemConfigurationDependentCriteriaService.createItemConfiguration(req.body);
    ...
    downloadItemConfigurationItemTypeExcel: async (req, res) => {
        try {
            ...
            await generateAndDownloadExcel(res, 'ItemConfigurationDependentCriteria', columns, results, 'ItemConfigurationDependentCriteria.xlsx');
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    }
...
export default ItemConfigurationDependentCriteriaControllers;
